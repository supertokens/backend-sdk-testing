...
describe(`mfa-recipeFunctions: ${printPath("[test/mfa/mfa.recipeFunctions.test.js]")}`, function () {
    ...
    it("test getFactorsSetupForUser with linked accounts", async function () {
        ...
        await AccountLinking.createPrimaryUser(SuperTokens.convertToRecipeUserId(user1.user.id));
        await AccountLinking.linkAccounts(SuperTokens.convertToRecipeUserId(user2.user.id), user1.user.id);
    ...
    it("test getMFARequirementsForAuth with passwordless otp-email", async function () {
        ...
        for (const tc of testCases) {
            let requirements = await MultiFactorAuth.recipeImplementation.getMFARequirementsForAuth({
                user: user.user,
